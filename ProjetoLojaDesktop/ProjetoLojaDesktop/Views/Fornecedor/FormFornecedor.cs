...
namespace ProjetoLojaDesktop
{
    public partial class FormFornecedor : Form
    {
        ...
        private void carregarGridEndereco()
        {
            ...
            dgvEndereco.Columns["idEndereco"].Visible = false;
        ...
        private void carregarGridTelefone()
        {
            ...
            dgvTelefone.DataSource = telefonedata.obterObjTelefoneDaPessoa(idFornEditado);

            dgvTelefone.Columns["idPessoa"].Visible = false;
            dgvTelefone.Columns["idTelefone"].Visible = false;

            dgvTelefone.Columns["numero"].HeaderText = "NUMERO";
            dgvTelefone.Columns["descricao"].HeaderText = "TIPO";

            //dgvTelefone.Columns["Pessoa"].Visible = false;
            //dgvTelefone.Columns["TipoTelefone"].Visible = true;


            //c.idPessoa, c.idTelefone, c.numero, c.TipoTelefone.descricao
        }
        ...
        private void btnDesatiivarPesq_Click(object sender, EventArgs e)
        {
            ...
            if (dgvPesquisar.Rows.Count > 0)
            {
                if (MessageBox.Show("Deseja realmente excluir o registro selecionado?", "Confirmação", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                    == DialogResult.Yes)
                {
                    int idpessoa = Convert.ToInt32(dgvPesquisar.CurrentRow.Cells["idPessoa"].Value);
                    ender.excluirTodosEnderecosPessoa(idpessoa);
                    //telefonedata.excluirTodosTelefonePessoa(idpessoa);
                    PessoaJuridica Pj = pJus.obterPessoaJuridica(idpessoa);
